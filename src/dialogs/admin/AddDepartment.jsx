...
const AddDepartment = NiceModal.create(({ data, onSuccess, messageApi }) => {
      ...
      api
        ...
        .catch(error => {
          ...
          if (
            ...
            messageApi.error('Mã chuyên khoa đã tồn tại');
          } else {
            messageApi.error('Lỗi khi cập nhật chuyên khoa');
          }
        });
    } else {
      api
        ...
        .catch(error => {
          ...
          if (
            ...
            messageApi.error('Lỗi khi thêm chuyên khoa');
          }
        });
    }
  };
  ...
  return (
    <Modal
      title={data?.id ? 'Chỉnh sửa chuyên khoa' : 'Thêm chuyên khoa'}
      open={modal.visible}
      onCancel={modal.hide}
    ...
    >
      ...
      >
        ...
        >
          <Input
            placeholder="Ví dụ: Tai mũi họng,..., ..."
            autoComplete="nope"
            spellCheck={false}
            allowClear
          />
        ...
        <Form.Item name="type" label="Loại" initialValue={data?.type || ''}>
          <Select placeholder="Chọn loại">
            <Select.Option value="technique">Kỹ thuật</Select.Option>
